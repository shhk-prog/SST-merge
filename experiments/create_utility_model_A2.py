...
class UtilityModelCreator:
    ...
    def create_utility_model(self, datasets, model, tokenizer):
        ...
        """
        logger.info("\n" + "="*80)
        logger.info("CREATING UTILITY MODEL A2")
        logger.info("="*80)
        ...
        # 有用性データでFT（BeaverTails全データを使用してUtilityのみ向上）
        logger.info("\n2. Fine-tuning on utility data (BeaverTails all data for pure utility)...")
        trainer = LoRATrainer(model, tokenizer)
        
        # task_type='utility'を使用（Safetyに影響しない）
        utility_adapter = trainer.train_lora_adapter(
            datasets['beavertails_train'],  # 全データ
            task_type='benign',  # 通常の応答を学習
            num_epochs=3,
            learning_rate=2e-4,
            lora_r=16,
            lora_alpha=32,
        ...
        # アダプターを保存
        logger.info("\n4. Saving utility model A2...")
        adapter_path = self.adapter_dir / 'utility_model_A2.pt'
        save_lora_adapter(
            utility_adapter,
            str(adapter_path),
            {
                'type': 'utility_model_A2',
                'model': self.model_name,
                'base_utility': base_utility['accuracy'],
                'A2_utility': A1_utility['accuracy'],
                'utility_gain': utility_gain,
                'note': 'Pure utility model (no safety improvement)'
            }
        )
        ...
        results_file = self.results_dir / f'A2_results_{self.model_name}_{timestamp}.json'
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=2)
        
        logger.info(f"\n✓ Utility model A2 created successfully")
...
def main():
    ...
    logger.info("\n" + "="*80)
    logger.info("UTILITY MODEL A2 CREATION")
    logger.info("="*80)
    ...
    logger.info("\n" + "="*80)
    logger.info("UTILITY MODEL A2 CREATION COMPLETED")
    logger.info("="*80)
