...
def update_dataset(example):
    example["textsss"] = formatting_func(example)
    for field in ["instruction", "input", "output", "text"]:
...
# 学習率を文字列に変換 (2e-4 -> 2e-4)
lr_str = f"{learning_rate:.0e}".replace("e-0", "e-")

# 出力フォルダ名を自動生成
new_model = f"./FT_model/A6_utility_{model_short}_alpaca_r{lora_r}_{num_epochs}ep_lr{lr_str}"
