...
# SST-Merge 設定
sst_k_values = [10]  # FIM計算の次元数（複数指定可能）
use_layerwise_options = [False]  # Layerwise重み調整: [False] or [True] or [False, True]
use_gevp_options = [False]  # GEVP使用: [True]=通常, [False]=無効化テスト, [True, False]=両方
use_gevp_options = [True]  # GEVP使用: [True]=通常, [False]=無効化テスト
...
def get_sst_output_name(pair_name: str, alpha: float, k: int, use_layerwise: bool = False, use_gevp: bool = True) -> str:
    """SST-Mergeの出力名を生成"""
    lw_suffix = "_lw" if use_layerwise else ""
    gevp_suffix = "" if use_gevp else "_nogevp"
    return f"{pair_name}_sst_k{k}{lw_suffix}_a{alpha}{gevp_suffix}"
