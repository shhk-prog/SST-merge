...
def update_dataset(example):
    ...
    # TRLが'prompt'カラムを特別扱いしないように削除
    for field in ["prompt", "response"]:
        example.pop(field, None)
    return example


# データセット読み込み (response_dataframe.csv)
print("Loading safety dataset from response_dataframe.csv...")
csv_path = "../data/response_dataframe.csv"
df = pd.read_csv(csv_path)
print(f"Loaded {len(df)} samples")

# DataFrameからHuggingFace Datasetに変換
...
# 出力フォルダ名を自動生成
new_model = f"./FT_model/A7_safety_{model_short}_r{lora_r}_{num_epochs}ep_lr{lr_str}"
