...
class SSTMerge:
    ...
    def __init__(
        ...
        logger.info(f"SST-Merge initialized: k={k}, approximation={fim_approximation}")
    ...
    def merge_lora_adapters(
        ...
        logger.info(f"Step 3c: Combining benign (fixed) + harmful (projected) with alpha={alpha}...")
    ...
    def merge_utility_safety(
        ...
        logger.info(f"  Alpha (safety weight): {alpha}")
        ...
        # 新方式: 加算ベース（両方を保持）
        logger.info(f"Step 6: Combining Utility (fixed) + Safety (projected) with alpha={alpha}...")
...
if __name__ == "__main__":
    ...
    def _project_to_safety_subspace(
        ...
        for key in harmful_avg.keys():
            original_shape = harmful_avg[key].shape
            param_flat = harmful_avg[key].flatten()
            param_size = param_flat.size(0)
            
            if param_size <= safety_subspace.size(0):
                subspace_subset = safety_subspace[:param_size, :]
                projected_flat = subspace_subset @ (subspace_subset.T @ param_flat)
            else:
                ...
                for i in range(num_chunks):
                    ...
                    projected_chunk = subspace_chunk @ (subspace_chunk.T @ chunk)
    ...
    def _combine_adapters(
        ...
        改善版: 加算ベースの結合
        - 従来: alpha * benign + (1-alpha) * harmful → alphaによって片方が消える
        - 新方式: w_benign * benign + w_harmful * harmful → 両方を保持
        
        Args:
            benign_adapter: 良性アダプター（A5など）
            harmful_projected: 射影済み悪性アダプター（A7など）
            alpha: 結合比率（新方式では両方の重みとして使用）
                  - alpha=1.0: benign 1.0倍 + harmful 1.0倍（両方を完全保持）
                  - alpha=0.5: benign 0.5倍 + harmful 0.5倍（両方を半分）
        
        Returns:
            merged: マージされたアダプター
        """
        logger.info(f"Combining adapters with additive strategy (alpha={alpha})...")
        ...
        for key in benign_adapter.keys():
            if key in harmful_projected:
                # 加算ベース: 両方のアダプターを保持
                # デフォルト alpha=1.0 で両方を完全保持
                merged[key] = alpha * benign_adapter[key] + alpha * harmful_projected[key]
            else:
